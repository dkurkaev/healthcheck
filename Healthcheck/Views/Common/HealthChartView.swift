...
struct HealthChartView: View {
    ...
    init(selectedArea: BodyArea?, period: StatisticsTimePeriod, isLockedToArea: Bool = false) {
        ...
        HStack(alignment: .center, spacing: 8) {
            HStack(alignment: .lastTextBaseline, spacing: 8) {
                if let date = selectedDate {
                    let value = getValue(at: date, from: ratingsForChart)
                    Text(String(format: "%.1f", value))
                        .font(.system(size: 32, weight: .bold, design: .rounded))
                    Text(period == .day ? date.timeString : date.relativeString)
                        .font(.subheadline)
            ...
            Spacer()
            if period == .day, let date = selectedDate, let event = findClosestEvent(to: date, area: isLockedToArea ? selectedArea : internalSelectedArea) {
                HStack(spacing: 4) {
                    Text(event.emoji)
                    Text(event.name)
                        .font(.caption)
    ...
    private func prepareChartData(for area: BodyArea?) -> [ChartPoint] {
        ...
        let filteredRatings: [BodyAreaRating]
        if let area = area {
            filteredRatings = allRatings.filter { $0.bodyArea?.name == area.name && $0.rating > 0 }
        } else {
            filteredRatings = allRatings.filter { $0.rating > 0 }
        }
        
        let periodRatings = filteredRatings.filter { $0.timestamp >= cutoff && $0.timestamp <= endDate }
        var points: [ChartPoint] = []
        
        func getAverageValue(at targetDate: Date, ratings: [BodyAreaRating]) -> Double? {
            let interval: TimeInterval = 300
            let groupedByTime = Dictionary(grouping: ratings) { 
                Date(timeIntervalSince1970: floor($0.timestamp.timeIntervalSince1970 / interval) * interval)
            }
            let sortedTimes = groupedByTime.keys.sorted()
            if let closestTime = sortedTimes.last(where: { $0 <= targetDate }),
               let latestRatings = groupedByTime[closestTime] {
                return Double(latestRatings.map(\.rating).reduce(0, +)) / Double(latestRatings.count)
            }
            return nil
        }
        
        let sortedInPeriod = periodRatings.sorted(by: { $0.timestamp < $1.timestamp })
        if let firstInPeriod = sortedInPeriod.first, firstInPeriod.timestamp > cutoff {
            let startValue = getAverageValue(at: cutoff, ratings: filteredRatings.filter { $0.timestamp < cutoff }) 
                          ?? (Double(firstInPeriod.rating))
            points.append(ChartPoint(date: cutoff, value: startValue))
        } else if periodRatings.isEmpty {
            if let lastValue = getAverageValue(at: cutoff, ratings: filteredRatings.filter { $0.timestamp < cutoff }) {
                points.append(ChartPoint(date: cutoff, value: lastValue))
                points.append(ChartPoint(date: endDate, value: lastValue))
                return points
            }
        }
        ...
        if period == .day {
            if area != nil {
                points.append(contentsOf: periodRatings.map { ChartPoint(date: $0.timestamp, value: Double($0.rating)) })
            } else {
                let interval: TimeInterval = 300
                let grouped = Dictionary(grouping: periodRatings) { rating in
                    Date(timeIntervalSince1970: floor(rating.timestamp.timeIntervalSince1970 / interval) * interval)
                }
                points.append(contentsOf: grouped.map { (date, ratings) in
                    let avg = Double(ratings.map(\.rating).reduce(0, +)) / Double(ratings.count)
                    return ChartPoint(date: date, value: avg)
                })
            }
        } else {
            let grouped = Dictionary(grouping: periodRatings) { calendar.startOfDay(for: $0.timestamp) }
            points.append(contentsOf: grouped.map { (date, ratings) in
                let avg = Double(ratings.map(\.rating).reduce(0, +)) / Double(ratings.count)
                return ChartPoint(date: date, value: avg)
            })
        ...
        var sortedPoints = points.sorted { $0.date < $1.date }
        if let lastPoint = sortedPoints.last, lastPoint.date < endDate {
            sortedPoints.append(ChartPoint(date: endDate, value: lastPoint.value))
        }
        return sortedPoints
    }
    
    private func getValue(at date: Date, from points: [ChartPoint]) -> Double {
        guard !points.isEmpty else { return 0 }
        let sorted = points.sorted { $0.date < $1.date }
        
        // Find the last point that is less than or equal to the selected date
        let closestPoint = sorted.last(where: { $0.date <= date })
        return closestPoint?.value ?? sorted[0].value
    }
...
enum StatisticsTimePeriod: String, CaseIterable {
    case day = "День"
    case week = "Неделя"
    case month = "Месяц"
